sobjs.list.rna.filtered = readRDS("data/nFeature_RNA_500_nFeature_RNA_5000_percent.mt_30_log10GenesPerUMI_0.80.sobj.list.citeSeq.RDS")
...
res0.4 = DimPlot(sobj.sct , group.by = "SCT_snn_res.0.4", label = T, label.size = 5)

Idents(object = sobj.sct) <- sobj.sct$SCT_snn_res.0.5
res0.5 = DimPlot(sobj.sct , group.by = "SCT_snn_res.0.5", label = T, label.size = 5)
...
saveRDS(sobj.sct, file = "data/good_samples__doublet__filtered__sobj.sct___Cite__Seq__RNA__sct__harmony__ndim50__res0.4_0.5_0.6_0.8.RDS")

sct.meta = sobj.sct@meta.data
write.csv(sct.meta, file = "results/sct.meta.postIntegration.csv", quote = F)
